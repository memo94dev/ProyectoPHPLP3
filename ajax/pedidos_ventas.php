...
?>
        <div class="table-responsive">
            <table class="table table-bordered table-hover table-striped">
                ...
                while ($row = mysqli_fetch_array($query)) {
                    ...
                    $sql_stock = mysqli_query($mysqli, "SELECT cantidad FROM stock WHERE cod_producto = '$cod_producto' AND cod_deposito = '$cod_deposito'");
                    ...
                    <tr>
                        ...
                        <td class="col-xs-1">
                            <input type="text" class="form-control" style="text-align:right" id="cantidad_<?php echo $cod_producto; ?>" value="1"
                                min="1"
                                max="<?php echo ($cantidad_stock > 0) ? $cantidad_stock : 0; ?>"
        ...
        <script>
            document.querySelectorAll("input[id^='cantidad_']").forEach(input => {
                input.addEventListener("input", function() {
                    let max = parseInt(this.max);
                    let val = parseInt(this.value);
                    if (val > max) {
                        alert("No hay stock suficiente! stock disponible:" + max)
                        this.value = max; // fuerza al máximo permitido
                    }
                    if (val < 1) {
                        this.value = 1; // fuerza al mínimo
                    }
                });
            });
        </script>
